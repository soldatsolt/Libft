...
static int	merger(char **store, char *note)
{
	...
	temporary = *store;
	*store = c_ft_strjoin(*store, note);
	garbage_free_this((void *)temporary);
	return (0);
}
...
static int	updating(char **store)
{
	...
	temporary = *store;
	*store = c_ft_strsub(*store, counter(*store) + 1,
...
int			c_get_next_line(const int fd, char **line)
{
	...
	if (!line || fd < 0 ||
		!(note = (char *)c_malloc(BUFF_SIZE + 1)) || read(fd, note, 0))
		return (-1);
	if (!store[fd])
		store[fd] = c_ft_strnew(0);
	...
	*line = c_ft_strsub(store[fd], 0, counter(store[fd]));
	if (to_delete(&store[fd]) == 0)
	{
		garbage_free_this((void *)note);
		return (0);
	}
	updating(&store[fd]);
	garbage_free_this((void *)note);
